use crate::{display::{BitmapVgaWriter, switch_graphics_mode, VgaModeSwitch, VgaPalette, VgaPaletteColor, STATIC_VGA_WRITER, restore_text_mode_font}, interrupt::setup::global_os_time, disable_cursor};
...
pub fn show_lars() {
    ...
    g_formatter.set_palette(palette.fade_factor(128));
    ...
    while unsafe { global_os_time } < timestamp + duration {
        ...
        if !visible_range.contains(&time) {
            if time < visible_range.start {
                fade = ((time - total_range.start) * u8::MAX as u64 / (visible_range.start - total_range.start)) as u8;
            }
            if time >= visible_range.end {
                fade = u8::MAX - ((time - visible_range.end) * u8::MAX as u64 / (total_range.end - visible_range.end)) as u8;
            }
        }
        if old_fade != fade {
